...
const INVOICE_ENQUIRY = {
  purchase: `
    SELECT COALESCE(e.enquiry_no, oe.enquiry_no) AS enquiry_no
     ...
     WHERE i.id = ?`,
  sales: `
    SELECT COALESCE(e.enquiry_no, oe.enquiry_no, qe.enquiry_no) AS enquiry_no
      FROM sales_invoices i
      LEFT JOIN enquiries e ON e.id = i.enquiry_id
      LEFT JOIN sales_orders so ON so.id = i.so_id
      LEFT JOIN enquiries oe ON oe.id = so.enquiry_id
...
const PAYMENT_ENQUIRIES_SQL = `(
  SELECT group_concat(ref) FROM (
    SELECT DISTINCT COALESCE(be.enquiry_no, boe.enquiry_no,
                             sie.enquiry_no, soe.enquiry_no, se.enquiry_no) AS ref
      ...
      LEFT JOIN enquiries sie ON sie.id = si.enquiry_id
      LEFT JOIN sales_orders so ON so.id = si.so_id
      LEFT JOIN enquiries soe ON soe.id = so.enquiry_id
      LEFT JOIN sales_quotations sq ON sq.id = so.quotation_id
      LEFT JOIN enquiries se ON se.id = sq.enquiry_id
     WHERE a.payment_id = p.id
       AND COALESCE(be.enquiry_no, boe.enquiry_no,
                    sie.enquiry_no, soe.enquiry_no, se.enquiry_no) IS NOT NULL
  ))`;
